 ...
 * @hide
 */
public class MediaControlVideo implements SurfaceTexture.OnFrameAvailableListener, Runnable {
    private static final String TAG = "MediaControlVideo";
    private static final boolean TRACE = false;         // Systrace
    private static final boolean DEBUG = true;       // Lots of logging
    ...
    private EglSurfaceWrapper mInputWindowSurface;
    private EglShareCore mEglShareCore;
    private FullFrameRect mFullScreen;
    private int mTextureId;
    private int mFrameNum;
    private MuxWrapVideo mVideoEncoder;
    ...
    private EglStateSaver mEglStateSaver;
    ...
    private final Object mStateLock = new Object();             // guards ready/running
    ...
    private SurfaceRenderer mDisplayRenderer;
    ...
    public MediaControlVideo(SessionConfig config) {
    ...
    public void reset(SessionConfig config) {
        ...
        synchronized (mStateLock) {
            while (mState != STATE.INITIALIZED) {
                try {
                    mStateLock.wait();
    ...
    private void handleReset(SessionConfig config) throws IOException {
        if (DEBUG) {
            Log.i(TAG, Thread.currentThread() + "handleReset before");
        }
        ...
        prepareEncoder(mEglStateSaver.getSavedEGLContext(),
        ...
        if (DEBUG) {
            Log.i(TAG, Thread.currentThread() + "handleReset over");
        }
...
//        mDisplayRenderer.signalVertialVideo(orientation);
        ...
        if (mFullScreen != null) mFullScreen.adjustForVerticalVideo(orientation, false);
        mDisplayRenderer.signalVertialVideo(orientation);
    }

    public void logSavedEglState() {
        mEglStateSaver.logState();
    }
     ...
     * Called from UI thread
     *
     * @return is the MediaControlVideo in the recording state
     */
    ...
    public void startRecording() {
        Log.i(TAG, mState.toString() + "  startRecording");
    ...
    private void startEncodingThread() {
        synchronized (mStateLock) {
            ...
            mRunning = true;
            new Thread(this, "MediaControlVideo").start();
            while (!mReady) {
                try {
                    mStateLock.wait();
     ...
     * After this call this instance of MediaControlVideo is no longer usable.
    ...
    private void handleFrameAvailable(SurfaceTexture surfaceTexture) {
        ...
        synchronized (mReadyForFrameFence) {
            if (!mReadyForFrames) {
                if (DEBUG) Log.i(TAG, "Ignoring available frame, not ready");
                return;
            }
            mFrameNum++;
            if (DEBUG && (mFrameNum % 30 == 0)) Log.i(TAG, "handleFrameAvailable");
            ...
            if (mRecording) {
                ...
                if (mEosRequested) {
                    Log.i(TAG, "Loop into UNINITIALIZED STATUS");
    ...
    public void onHostActivityPaused() {
        ...
        synchronized (mReadyForFrameFence) {
            ...
            if (!mRecording && mSurfaceTexture != null) {
                if (DEBUG) Log.i("CameraRelease", "Releasing camera");
                if (mDisplayView != null)
                    releaseDisplayView();
                mHandler.sendMessage(mHandler.obtainMessage(MSG_RELEASE_CAMERA));
            }
    ...
    public void onSurfaceCreated(int textureId) {
        if (DEBUG) Log.i(TAG, "onSurfaceCreated. Saving EGL State");
        synchronized (mStateLock) {
    ...
    private void handleSetSurfaceTexture(int textureId) throws IOException {
        synchronized (mSurfaceTextureFence) {
            if (mSurfaceTexture != null) {
                ...
                mFullScreen.release();
                mEglShareCore.release();

                // Create a new EGLContext and recreate the window surface.
                mEglShareCore = new EglShareCore(mEglStateSaver.getSavedEGLContext(), EglShareCore.FLAG_RECORDABLE);
                mInputWindowSurface.recreate(mEglShareCore);
                mInputWindowSurface.makeCurrent();
                ...
                //mEglStateSaver.makeNothingCurrent();
            } else {
    ...
    private void prepareEncoder(EGLContext sharedContext, int width, int height, int bitRate,
        ...
        mVideoEncoder = new MuxWrapVideo(width, height, bitRate, muxer);
        if (mEglShareCore == null) {
            // This is the first prepare called for this MediaControlVideo instance
            mEglShareCore = new EglShareCore(sharedContext, EglShareCore.FLAG_RECORDABLE);
        }
        if (mInputWindowSurface != null) mInputWindowSurface.release();
        mInputWindowSurface = new EglSurfaceWrapper(mEglShareCore, mVideoEncoder.getInputSurface());
        mInputWindowSurface.makeCurrent();
    ...
    private void openCamera(int desiredWidth, int desiredHeight, int requestedCameraType) {
        ...
        if (DEBUG) Log.i(TAG, "Camera preview set: " + previewFacts);
    }
    ...
    private void releaseCamera() {
        if (mDisplayView != null)
            releaseDisplayView();
        if (mCamera != null) {
            if (DEBUG) Log.d(TAG, "releasing camera");
            mCamera.stopPreview();
            mCamera.release();
            mCamera = null;
        }
    ...
    private static class EncoderHandler extends Handler {
        private WeakReference<MediaControlVideo> mWeakEncoder;

        public EncoderHandler(MediaControlVideo encoder) {
            mWeakEncoder = new WeakReference<MediaControlVideo>(encoder);
        }
        ...
        public void handleMessage(Message inputMessage) {
            ...
            MediaControlVideo encoder = mWeakEncoder.get();
            if (encoder == null) {
